...
6. Edit source only with the host's workspace tools. Flowpeek does not expose repository-source writes, arbitrary shell execution, credentials, deployments, or target-application execution.
...
8. Run repository-owned tests and checks with approved host tools. Flowpeek's related-test results are static candidates, not test-success evidence.
