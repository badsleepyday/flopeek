...
function writeDelta(root, from, payloadBytes = 128) {
  const target = path.join(root, ".flowpeek", "deltas", `${from}-${from + 1}.json`);
  fs.mkdirSync(path.dirname(target), { recursive: true });
  fs.writeFileSync(target, JSON.stringify({ schemaVersion: "flowpeek-delta/v1", projectId: "project:retention", fromGraphVersion: from, toGraphVersion: from + 1, payload: "x".repeat(payloadBytes) }));
}

test("delta history uses deterministic version and byte retention with a dry-run-first prune", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "flowpeek-delta-retention-"));
  try {
    for (let version = 1; version <= 12; version += 1) writeDelta(root, version, 256);
    const malformed = path.join(root, ".flowpeek", "deltas", "notes.json");
    ...
    assert.equal(fs.existsSync(path.join(root, ".flowpeek", "deltas", "1-2.json")), true);
    const applied = pruneGraphDeltas(root, { keepDeltas: 3, maxBytes: 10_000, dryRun: false });
    assert.equal(applied.dryRun, false);
    assert.equal(applied.pruned.length, 9);
    assert.equal(fs.existsSync(path.join(root, ".flowpeek", "deltas", "1-2.json")), false);
    assert.equal(fs.existsSync(path.join(root, ".flowpeek", "deltas", "12-13.json")), true);
...
test("a missing old Context Ref is explicit as expired when retained delta history was pruned", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "flowpeek-context-expiry-"));
  try {
    writeDelta(root, 5);
...
test("delta prune recovery rolls back an interrupted staging journal and completes a committed journal", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "flowpeek-delta-recovery-"));
  try {
    writeDelta(root, 1);
    const deltas = path.join(root, ".flowpeek", "deltas");
