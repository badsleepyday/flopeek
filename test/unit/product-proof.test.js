...
test("orientation proof validates raw per-case totals and keeps human and agent studies unrun", () => {
  ...
  assert.equal(evidence.flopeek.summary.flowSteps.matchedInExpectedOrder, 14);
  assert.equal(evidence.flopeek.summary.relatedTests.matched, 3);
  assert.equal(evidence.flopeek.summary.staleContextDetection.detected, 3);
  assert.equal(evidence.evidenceClasses.humanStudy, "not-run");
  assert.equal(evidence.evidenceClasses.agentStudy, "not-run");

  const raw = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "..", "benchmarks", "orientation-flopeek.json"), "utf8"));
  raw.summary.correctTargetRetrieval.matched -= 1;
  assert.throws(() => validateOrientationReport(raw, "flopeek"), /target totals are inconsistent/);

  const flowDrift = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "..", "benchmarks", "orientation-flopeek.json"), "utf8"));
  flowDrift.summary.flowSteps.matchedInExpectedOrder -= 1;
  assert.throws(() => validateOrientationReport(flowDrift, "flopeek"), /flow totals are inconsistent/);

  const claimDrift = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "..", "benchmarks", "orientation-flopeek.json"), "utf8"));
  claimDrift.studyEvidence.agentStudy.status = "measured";
  assert.throws(() => validateOrientationReport(claimDrift, "flopeek"), /must not imply an executed human or agent study/);
});

test("product proof combines bounded public evidence with current repository facts and explicit non-claims", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "flopeek-product-proof-"));
  try {
    ...
    assert.match(proof.localBenchmark.command, /^flopeek proof/);
...
test("proof CLI returns the versioned report with an opt-in local benchmark", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "flopeek-proof-cli-"));
  try {
