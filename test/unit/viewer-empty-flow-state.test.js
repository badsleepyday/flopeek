...
test("viewer keeps Canvas as the supported renderer and bounds the WebGL option as a preview", () => {
  ...
  assert.match(INDEX, /id="measure-renderer"/);
  assert.match(APP, /function measureRendererPair/);
  assert.match(APP, /crossDevice: \{ status: "unavailable", reason: "one-local-browser-session" \}/);
  assert.match(APP, /Canvas remains the supported default/);
});
