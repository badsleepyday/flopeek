...
test("native incremental coordinator reuses cross-process JS facts without changing the full compatibility projection", (t) => {
  const root = copiedFixture("legacy-handoff");
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));

  const jsFirst = scanRepository(root);
  const first = scanWithNativeIncremental(root, { native: NATIVE });
  ...
  const unchanged = scanWithNativeIncremental(root, { native: NATIVE });
  ...
  const changed = scanWithNativeIncremental(root, { native: NATIVE });
