...
test("development repository projects a bounded public tree without agent governance", () => {
  ...
  const audit = auditPublicFiles(policy.requiredPaths, policy, { name: "flowpeek", version: "0.2.0", private: true, license: "Apache-2.0" }, { totalBytes: 1000, sourceClean: true, revision: "a".repeat(40) });
  assert.equal(audit.structureStatus, "passed");
  assert.equal(audit.releaseReadiness.blockers.includes("license-file-missing"), false);
  assert.equal(audit.releaseReadiness.blockers.includes("package-license-missing"), false);
