...
test("portable Flowpeek tool skill preserves the evidence-first agent contract", () => {
  const skillRoot = path.join(root, "integrations", "skills", "flowpeek");
  ...
  assert.match(source, /Flowpeek does not expose repository-source writes/);
  assert.match(metadata, /\$flowpeek/);
