...
pub fn build_native_js_structural_records(
    ...
    let mut records = facts
        ...
        .map(|(path, facts)| {
            ...
            let (file_type, label, mut responsibility) = classify_file(path);
            if scope == "test" {
            ...
            let file_metadata = json!({
                ...
                "language": language,
                "analysis": facts.structural.analysis,
                "evidence": { "file": path },
...
pub fn build_native_js_entry_facts(
    ...
    package_commands.sort_by(|left, right| {
        left["manifest"]
            .as_str()
            .cmp(&right["manifest"].as_str())
            .then(
                left["scriptName"]
                    .as_str()
    ...
    json!({
        ...
        "entryPoints": {
            ...
            "supported":{"packageScripts":package_scripts,"djangoManagementCommands":[],"frameworkCommands":[],"nodeCronSchedules":schedules,"limitation":"Only an explicitly supported exact static subset becomes a Flow Lens entry: a direct package runner target, a supported Python framework command declaration, or a literal node-cron registration."},
            "unsupported":{"packageScripts":unsupported_package_scripts,"djangoManagementCommands":[],"frameworkCommands":[],"nodeCronSchedules":unsupported_schedules,"limitation":"Unsupported scripts, framework commands, and schedule registrations are static inventory only. Their absence from Flow Lenses does not prove they cannot run or have no behavior."},
            "limitations":[
...
#[cfg(test)]
mod tests {
    use super::{build_native_js_entry_facts, build_native_js_structural_records};
