...
const DEPTH_LIMITS = Object.freeze({
  summary: { pathAnchors: 2, features: 2, flows: 3, tests: 1 },
  standard: { pathAnchors: 5, features: 5, flows: 8, tests: 4 },
  evidence: { pathAnchors: 8, features: 8, flows: 12, tests: 8 },
});
...
function pathAnchorCandidates(request, nodesByPath) {
  return request.changedPaths.flatMap((changedPath) => {
    const candidates = [...(nodesByPath.get(changedPath) || [])].sort((left, right) => {
      const leftRank = left.kind === "file" ? 0 : 1;
      const rightRank = right.kind === "file" ? 0 : 1;
      return leftRank - rightRank || left.id.localeCompare(right.id);
    });
    const node = candidates[0];
    return node ? [{ id: node.id, node, path: changedPath }] : [];
  });
}
...
function relatedTests(graph, selectedFlows, selectedFeatures, intentWords) {
  ...
  return graph.nodes.filter((node) => node.type === "test" || node.kind === "test").map((node) => {
    const taskOverlap = overlapScore(intentWords, words(`${node.label} ${node.path || ""}`));
    let score = taskOverlap;
    const reasons = [];
    if (connectedTests.has(node.id)) { score += 5000; reasons.push("direct-static-edge-to-selected-flow"); }
    if (taskOverlap > 0) reasons.push("task-intent-token-overlap");
    if (relevantFeatures.has(featureKey(node)) && (connectedTests.has(node.id) || taskOverlap > 0)) {
      score += 1000;
      reasons.push("selected-feature");
    }
...
function createHandoffContext(graph, input = {}) {
  ...
  const pathAnchors = pathAnchorCandidates(request, nodesByPath);
  const features = featureCandidates(graph, request, intentWords, changedNodes).filter((candidate) => candidate.score > 0);
  const flows = flowCandidates(graph, request, intentWords, changedNodes, featuresByNode).filter((candidate) => candidate.score > 0);
  ...
  const packet = {
    ...
    included: { handoffWorkspace: null, pathAnchors: [], features: [], flows: [], tests: [], evidenceRefs: [], relevantDeltas: [] },
  ...
  // Exact caller-supplied paths outrank generic evidence-depth caps. They remain
  // bounded by the validated request maximum and the packet character budget.
  for (const candidate of pathAnchors) {
    const item = {
      id: candidate.node.id,
      label: candidate.node.label,
      path: candidate.path,
      contextRef: createContextRef(graph.project.projectId, "node", candidate.node.id, graph.state.graphVersion),
      relevance: { score: 10000, reasons: ["exact-changed-path"] },
      evidenceClass: "static-parser-fact",
    };
    packet.included.pathAnchors.push(item);
    if (!fits(packet, charLimit)) packet.included.pathAnchors.pop();
  }
  ...
  const refs = uniqueSorted([
    project.briefRef,
    ...packet.included.pathAnchors.map((item) => item.contextRef),
  ...
  packet.omitted.pathAnchors = omissionSummary(pathAnchors, new Set(packet.included.pathAnchors.map((item) => item.id)), "token-budget-or-evidence-depth-limit");
  const omittedGroups = [
    [packet.omitted.pathAnchors, pathAnchors.map((item) => item.id).filter((id) => !packet.included.pathAnchors.some((item) => item.id === id))],
  ...
  while (packet.budget.estimatedCharacterCount > charLimit) {
    ...
    } else if (packet.omitted.pathAnchors.omittedIds.length) {
      packet.omitted.pathAnchors.omittedIds.pop();
      packet.omitted.pathAnchors.omittedIdsNotListed += 1;
      addReason(packet.omitted.pathAnchors, "omitted-id-list-truncated-to-respect-budget");
    ...
    else if (demoteIncludedItem(packet.included.pathAnchors, packet.omitted.pathAnchors)) {}
