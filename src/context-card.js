...
function historyPrunedBefore(options, graphVersion) {
  const history = options.deltaHistory?.();
  const oldest = history?.retained?.reduce((minimum, item) => Math.min(minimum, item.fromGraphVersion), Infinity);
  return Number.isFinite(oldest) && graphVersion < oldest ? { oldestRetainedFrom: oldest, newestRetainedTo: history.retained[0]?.toGraphVersion || null } : null;
}

function expiredHistoryResolution(value, kind, retention) {
  return {
    status: "expired",
    requestedRef: value,
    resolvedRef: null,
    card: null,
    successorCandidates: [],
    reason: `The requested ${kind} Context Ref predates retained adjacent delta history and cannot be reconstructed.`,
    code: "history-pruned",
    retention,
  };
}
...
function resolveNodeContextRef(graph, parsed, options, value) {
  ...
  const retention = historyPrunedBefore(options, parsed.graphVersion);
  if (retention) return expiredHistoryResolution(value, "node", retention);
...
function resolveFlowContextRef(graph, parsed, options, value) {
  ...
  const retention = historyPrunedBefore(options, parsed.graphVersion);
  if (retention) return expiredHistoryResolution(value, "flow", retention);
  const delta = options.readDelta?.(parsed.graphVersion, parsed.graphVersion + 1) || null;
