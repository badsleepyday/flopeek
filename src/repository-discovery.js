...
function verifyAnalysisPlan(inputRoot, analysisPlan) {
  ...
  for (const adapter of registry.adapters) for (const extension of adapter.extensions) extensionAdapters.set(extension, adapter);
  ...
  for (const directoryPath of analysisPlan.directories) {
    ...
    for (const entry of entries) {
      ...
      const adapter = extensionAdapters.get(path.extname(entry.name).toLowerCase());
