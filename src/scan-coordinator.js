...
function graphIdentity(graph, source, freshness) {
  return graph ? {
    available: true,
    source,
    freshness,
...
function createScanCoordinator(inputRoot, options = {}) {
  ...
  let outcome = {
    ...
    discovery: null,
    activeGraph: graphIdentity(null, "none", "unavailable"),
  ...
  const finalize = (operationId, startedAt, status, reason, active, details = {}) => {
    ...
    outcome = {
      ...
      activeGraph: graphIdentity(active.graph, active.source, details.freshness || (status === "complete" ? "current" : "stale-unverified")),
      cachePromotion: {
