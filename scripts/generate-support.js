...
const block = [
  ...
  ...registry.adapters.map((adapter) => {
    const fileTypes = [...adapter.extensions, ...(adapter.filenames || [])].join(" ");
    return `| ${cell(adapter.id)} | ${cell(`${adapter.languages.join(", ")} / ${fileTypes}`)} | ${cell(adapter.parser)} | ${cell(adapter.availability)} | ${level(adapter, "structure")} | ${level(adapter, "imports")} | ${level(adapter, "directCalls")} | ${cell(adapter.requiredToolchain || "None")} |`;
  }),
  "",
