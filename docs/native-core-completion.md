...
- Current verified implementation SHA: `03121661ef5ccf89441843f9be241080396b5193`
...
| --- | --- | --- | --- | --- |
| 1. Native correctness and incremental matrix | `passing` | `2e2fe76` | `cargo test --locked --manifest-path native/flopeek-core/Cargo.toml`; `node --test test/unit/core-client.test.js`; `node --test test/unit/native-incremental-coordinator.test.js` | Persistent and cache-disabled eight-adapter matrix is asserted by `test/unit/core-client.test.js`; all acceptance commands passed locally and in six-cell CI run `30575096710`. |
| 2. Exact adapter parity | `passing` | `050bcb1`, `b310137` | `npm run verify:native-adapter-parity` | `flopeek-native-adapter-parity/v1`, 8 adapters and 29/29 exact cases. Raw per-cell artifacts are attached to CI run `30575096710`; final candidate path: `adapter-parity.json`. |
...
| 4. CI, candidate, and promotion separation | `blocked` | `41d61db`, `5b164d3` | six-cell `Verify`; `.github/workflows/native-candidate.yml`; `.github/workflows/native-promotion.yml` | Source CI is green at run `30575096710`. Dispatching `.github/workflows/native-candidate.yml` for exact source SHA `03121661ef5ccf89441843f9be241080396b5193` returned GitHub API `404 workflow not found on the default branch`. The default branch is `main`, and neither native workflow is registered there yet. A repository merge/default-branch update is required before GitHub can dispatch the six-platform candidate; no local run is accepted as a substitute. |
