...
This protocol turns Flopeek's deterministic semantic suggestions into a small, privacy-safe human-review cohort. It does **not** turn feedback into business verification, runtime proof, or permission to train a model.
...
2. Inspect only the supporting local code needed to judge the technical wording. Do not copy it into Flopeek feedback or the dataset.
...
4. In Flow Lens, submit feedback with your reviewer pseudonym:
   ...
   - `abstained`: Flopeek correctly declined to suggest; mark the abstention `appropriate` in the private cohort. If enough static evidence existed for a conservative suggestion, mark it `unnecessary`.
...
Use at least two reviewer pseudonyms across the held-out cohort. For a quality check, have a second reviewer independently inspect a small random sample outside the dataset, resolve disagreements, and keep any discussion in the repository owner's approved system—not in Flopeek metadata.

Run the gate against the private file:

```powershell
npm.cmd run evaluate:reviewed-feedback -- --dataset C:\private\review.flopeek-reviewed.json --require-gate
```
