...
- any renderer evidence gap remains recorded under DF-022-P3.
...
available, the agent reconciliation stayed `unresolved`, and the comparison
stayed `unresolved`. The local Viewer displayed the selected stale overlay as
one planned node and zero planned relationships while keeping it separate from
technical source facts. Windows Node completed the real-stdio journey; the
selected Ubuntu WSL distro had no Linux Node/npm installed, so that cell is
explicitly unavailable rather than a product pass. This does not replace a
human reconciliation, accessibility review, or independent-provider evidence.

## Post-C11 dogfooding queue

1. DF-001 now has positive Windows static baselines from consented PHP and
   Node.js corpora: 1,629 analyzed PHP files completed cache-disabled in 17.7
   seconds and 306 Node.js files in 12.0 seconds. Keep the finding open until a
   corpus reproduces or bounds the original over-60-second large-workspace case.
2. DF-032 is closed by a consented, cache-backed static MCP replay: all six
   supplied PHP paths remained exact anchors at standard depth, while no target
   process was run. DF-035 is also closed: a temporary content-only legacy view
   edit produced `file-content-only` delta evidence and the bounded convention
   projection found its Education peer; the fixture was restored cleanly.
3. DF-028 is closed for the local JSON contract: cache status separates graph
   delta history and protected user metadata; a history prune is previewed by
   default, retains the latest adjacent delta, and requires `--apply` after
   staging behind a recovery journal. The default preview retains eight valid
   deltas within 16 MiB. Deterministic recovery tests and consented PHP/Node
   corpus previews completed without deleting corpus cache history. Reconsider
   a backend only after future evidence exceeds these declared bounds. DF-005
   now has a fixture-only Windows helper-process matrix: available Go/.NET
   helpers leave each observed PID stopped after completion, timeout, bounded
   cancellation, and two concurrent scans. A C# launcher timeout can occur
   before the managed helper reports a PID, so that row is explicitly
   `not observed`, not direct cleanup proof. A user-local Ubuntu 22.04 toolchain
   matrix passes for both helpers, and the manual GitHub Actions matrix passed on
   Ubuntu and macOS in run 30072804745. The selected default WSL environment
   still lacks Node/Go/.NET, but DF-005 is closed for the fixture-only helper
   lifecycle contract; it is not target-runtime evidence.
4. Keep DF-022's Canvas/WebGL corpus and cross-device measurement last. It is
   not a default-renderer decision gate until a pinned corpus exists.
