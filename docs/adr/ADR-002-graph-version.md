...
7. Flowpeek retains at most 40 newest adjacent deltas in `.flowpeek/deltas/`. It does not reconstruct arbitrary non-adjacent delta chains.
...
- An old version cannot be resolved to an arbitrary historical graph merely because an adjacent delta was retained; Git snapshots remain a separate feature.
...
- Orphaned delta files are never returned as the current delta unless their target version equals the current graph version.
