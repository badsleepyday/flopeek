...
Flopeek supports two serve modes. Per-project mode keeps one graph server for one repository. Global mode (`serve -g`) exposes one user-facing hub port and activates multiple independently identified project roots behind it. The browser selects one active project; ordinary API and SSE traffic is routed to that project's isolated scanner, graph, watcher, version, and `.flopeek` cache.
