...
Human verification requests carry the graph version and Flow Context Ref that were reviewed. Flowpeek rejects stale drafts with a conflict. Reusable semantic memory is a bounded read index over `.flowpeek/flow-verifications.json`; only current or compatible verification is reusable by default.

## Boundaries

- `.flowpeek` stores metadata, not an LLM runtime or model weights.
