...
Flopeek can now identify a project and a material static graph version, but a person or agent still needs a compact artifact that can be copied, interpreted, and checked for staleness. A graph node ID by itself is only a current-state key; it does not say which project or graph version supplied the evidence.
...
4. A node Context Card uses `flopeek-context/v1`. It includes project/graph identity, extracted technical responsibility, bounded parser evidence, direct incoming/outgoing relations, related static flows/tests, limitations, human-description state, and safe navigation/recommendation actions.
5. A Context Packet uses `flopeek-context-packet/v1` and is available as JSON or Markdown. It excludes source-file bodies, secrets, shell commands, credentials, deployment actions, and hidden model state.
6. A flow Context Card uses `flopeek-context/v1` with `kind: flow`. It packages one bounded `flopeek-flow-lens/v1` projection, direct related-test evidence, truncation, limitations, unresolved questions, and safe actions. Its title and technical summary are derived static descriptions, not business purpose.
...
- A human and an agent can hand off a bounded, graph-versioned node or flow context without sending source contents through Flopeek.
