...
This small repository exists only to demonstrate Flowpeek's current static evidence workflow. The showcase runner copies it into a temporary workspace and never executes this target application.
...
`src/checkout/discount.ts` contains a computed dynamic import on purpose. Flowpeek does not resolve that target and must not present the missing edge as proof that no runtime discount behavior exists.
