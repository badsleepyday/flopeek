...
| C11 Stabilization and dogfooding | `in progress` | The real-stdio-MCP journey now has one Flowpeek-on-Flowpeek local observation, including an explicit stale Viewer overlay and an agent-only unresolved reconciliation. Human/accessibility and independent-provider review evidence remain open before this sequence can close. |
