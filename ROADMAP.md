...
| C11 Stabilization and dogfooding | `partial` | Public Core CI at `2feb5ec` passed Ubuntu, Windows, and macOS on Node 20 and 22, including clean-room package/MCP verification. A real local stdio-MCP and single-host Viewer observation cover the bounded refresh/comparison journey. Human/accessibility and independent-provider review evidence remain open before this sequence can close. |
...
| S5 C11 observable stabilization | `partial` | The [six-job public Core matrix](https://github.com/badsleepyday/flowpeek/actions/runs/30155551679) passed on Ubuntu, Windows, and macOS with Node 20 and 22; real local stdio MCP and a single-host Viewer refresh/comparison observation also passed. Manual Viewer/readability/accessibility recovery checks and explicitly scoped independent-provider evidence remain required. |
